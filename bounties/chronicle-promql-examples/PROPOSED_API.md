...
The examples below use `--data-urlencode` so label matchers and range selectors are
encoded safely. Use `/api/v1/query` for an instant query and
`/api/v1/query_range` when you also provide `start`, `end`, and `step`.

| Use case | PromQL expression |
|----------|-------------------|
| Select every series for a metric | `http_requests_total` |
| Match one label | `http_requests_total{method="GET"}` |
| Match several labels | `http_requests_total{method="GET",status="200"}` |
| Exclude a label value | `http_requests_total{method!="POST"}` |
| Match label values with a regular expression | `http_requests_total{status=~"2..|3.."}` |
| Exclude label values with a regular expression | `http_requests_total{path!~"/health.*"}` |
| Select a five-minute range | `http_requests_total{method="GET"}[5m]` |
| Sum all matching series | `sum(http_requests_total{status="200"})` |
| Average by label | `avg by (host) (cpu_usage)` |
| Group a sum by two labels | `sum by (method, status) (http_requests_total)` |
| Find the maximum matching value | `max(temperature{location="outdoor"})` |
| Count matching series | `count(events{service=~"api|worker"})` |
| Calculate a five-minute rate | `rate(http_requests_total{method="GET"}[5m])` |
| Evaluate a selector at a Unix timestamp | `cpu_usage @ 1609459200` |
| Shift a selector one hour into the past | `cpu_usage offset 1h` |

Instant query:

```bash
curl --get http://localhost:8086/api/v1/query \
  --data-urlencode 'query=sum by (method) (http_requests_total{status=~"2..|3.."})' \
  --data-urlencode 'time=1609459200'
```

Range query:

```bash
curl --get http://localhost:8086/api/v1/query_range \
  --data-urlencode 'query=rate(http_requests_total{method="GET"}[5m])' \
  --data-urlencode 'start=1609459200' \
  --data-urlencode 'end=1609462800' \
  --data-urlencode 'step=60'
```

> Chronicle supports the selector, label-matcher, range, aggregation, `@`, and
> `offset` forms shown above. Keep expressions within the supported subset when
> moving queries from a full Prometheus deployment.
